...
def filter_frames_with_stats(frame_paths, hamming_distance_threshold=25, blur_threshold=50, brightness_threshold=50.0):
    ...
    for frame_path in frame_paths:
        try:
            ...
            if previous_hash is not None:
                ...
                # print(f"Hamming distance between frames: {distance}")
...
def process_batches(frame_batches, attributes):
    region_info_process_batches = []
    all_batch_results = []

    print("\n-------- Batch Process START --------")

    # process batch and combine all
    for batch_index, batch in enumerate(frame_batches):
        print(f"Batch {batch_index + 1} with {len(batch)} Frames ...")

        if len(batch) == 0:
            print(f"Batch {batch_index + 1} has no frames left after preprocessing! Skipping...")
            continue

        batch_result = analyze_image_batch(batch, attributes)
        all_batch_results.append(f"{batch_result}\n")

    print("-------- Batch Process END, Report Process START --------")

    if not all_batch_results:
        region_info_process_batches = [{'warning': ['nothing']}]
        print("No analysis results to summarize!")
        return region_info_process_batches
    ...
    for retry in range(max_retries):
        ...
        try:
            ...
            region_info_process_batches = [{'warning': ['invalid']}]
        ...
        for i, region in enumerate(house_evaluation.regions):
            validation_result = validate_region_data(region.model_dump())
            print(f"Report {retry + 1} region {i + 1}: {validation_result}")

            if validation_result == 0:
                all_valid = False
            else:
                temp_region_info.append(region.model_dump())

        if all_valid:
            region_info_process_batches.extend(temp_region_info)
...
def process_video(video_path, attributes):
    ...
    if len(frame_paths) != 0:
        ...
        print(f"Deleted similar frames: {stats['similar']}")
